...
export default function AddConfiguration() {
	...
	const { addConfiguration: Configuration } = modalAddProduct;
	...
	const handleAddInfor = () => {
		...
		setModalAddProduct(prev => ({
			...prev,
			addConfiguration: [...prev.addConfiguration, newInfor],
		}));
		setModalAddInfor({
			...modalAddInfor,
			inforKey: '',
	...
	const handleDelete = keyObj => {
		setModalAddProduct(prev => ({
			...prev,
			addConfiguration: prev.addConfiguration.filter(item => Object.keys(item)[0] !== keyObj),
		}));
	};
	const handleUpdate = () => {
		...
		// kiểm tra không được trùng key
		const checkIndex = Configuration.findIndex(item => Object.keys(item).includes(modalEditInfor.inforKey));
		if (checkIndex != -1) {
			return message.error('Tên giá trị đã có');
		}
		...
		setModalAddProduct(prev => ({
			...prev,
			addConfiguration: prev.addConfiguration.map(item => {
				if (Object.keys(item)[0] === modalEditInfor.inforKeyOld) {
					return {
						[modalEditInfor.inforKey]: modalEditInfor.value,
					};
				}
				return item;
			}),
